...
The project processes four different types of raw data and writes the results to `data/processed/`. Each pipeline follows the **Extract / Transform / Verify / Load** structure.

- **CSV** - reads the World Happiness CSV file, extracts the selected numeric column, calculates descriptive statistics, verifies the results, and writes the statistics to a text file. For my custom analysis, the selected column is **Healthy life expectancy**.

- **JSON** - reads the astronauts JSON file, extracts the list of people, counts people by spacecraft, verifies the results, and writes the counts to a text file.
